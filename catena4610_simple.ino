...
Author:
        Dhinesh Kumar Pitchai, MCCI Corporation	July 2024
...
// Flash parameters
FlashParamsStm32L0_t::ParamBoard_t gFlashParam;
...
void setup(void)
        ...
        for (uint8_t i = 0; i < len; i++)
          ...
          if(! CharIsPunctuation(VERSION[i]))
            {
            // ver[j] = (ver[j] << 4) + VERSION[i];
            saveVersion(VERSION[i], j);
            }
          else
            j++;
          }
        ...
        for (uint8_t n = 0; n < (len - 2); n++)
          gCatena.SafePrintf("%d-", sw_version[n]);
        /*unsigned long num;
        double ver;
        ver = std::stod(VERSION);
        for (num = strtoul (VERSION, '.', 0); VERSION !='.' VERSION != NULL
        gCatena.SafePrintf("\nversion: ");
        Serial.println(ver);*/
...
void setup_platform(void)
        ...
        /* gFlashParam.setAssembly(234001129);
        gFlashParam.setModel(4610);
        gFlashParam.setRev(2); */
        gCatena.SafePrintf(
                "Assembly:  %d\nModel: %d Rev - %c\n",
                gFlashParam.getAssembly(),
                gFlashParam.getModel(),
                gFlashParam.getRevChar()
                );
        }
...
void setup_uplink(void)
        ...
        if (!(gCatena.GetOperatingFlags() &
                ...
                if (! gLoRaWAN.IsProvisioned())
                        ...
                        Serial.print("AppEUI: ");
                        uint8_t eui[16];
                        gLoRaWAN.GetAppEUI(eui);;
                        for (int i = 0 ; i < 8 ; i++) {
                                Serial.print(eui[i],16);
                                }                
                        Serial.println();

                        Serial.print("AppKey: ");
                        uint8_t key[16];
                        gLoRaWAN.GetAppKey(key);;
                        for (int j = 0 ; j < 16 ; j++) {
